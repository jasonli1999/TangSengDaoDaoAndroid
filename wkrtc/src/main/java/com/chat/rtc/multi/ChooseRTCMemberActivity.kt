...
class ChooseRTCMemberActivity : WKBaseActivity<ChooseCallMembersLayoutBinding>() {
    ...
    override fun rightButtonClick() {
        ...
        if (selectedAdapter!!.data.size > 0) {
            ...
            if (WKRTCApplications.instance.iChooseMembersBack != null && !TextUtils.isEmpty(WKRTCApplications.instance.roomID) && WKReader.isNotEmpty(uidList)) {
                showTitleRightLoading()
                showOrHideRightBtn(false)
                RTCModel.getInstance().invokeJoinRoom(
                    WKRTCApplications.instance.roomID,
                    uidList
                ) { code, msg ->
                    showOrHideRightBtn(true)
                    hideTitleRightLoading()
                    if (code == HttpResponseCode.success.toInt()) {
                        WKRTCApplications.instance.iChooseMembersBack!!.onBack(uidList)
                        finish()
                    } else {
                        showToast(msg)
                    }
