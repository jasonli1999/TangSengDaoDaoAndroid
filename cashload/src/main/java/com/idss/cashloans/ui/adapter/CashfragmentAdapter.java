...
public class CashfragmentAdapter extends RecyclerView.Adapter<CashfragmentAdapter.ViewHolder> {
    private final Context mContext;
    private final List<HistoryOrdersModel.DataDTO.RecordsDTO> records;

    public CashfragmentAdapter(Context mContext, List<HistoryOrdersModel.DataDTO.RecordsDTO> records) {
    ...
    @NonNull
    @Override
    public CashfragmentAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.adapter_cashloan, parent, false);
        return new CashfragmentAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull CashfragmentAdapter.ViewHolder holder, int position) {
        HistoryOrdersModel.DataDTO.RecordsDTO recordsDTO = records.get(position);
        holder.tv_date.setText("申请日期: " + recordsDTO.getCreateTime());
        holder.tv_loans.setText(" ¥ " + recordsDTO.getApplicationAmount());
        /**
...
//        });
        ...
        holder.ll_application_progress.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(mContext, LoanProgressActivity.class);
                intent.putExtra("id", recordsDTO.getId());
                mContext.startActivity(intent);
            }
        });
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private final LinearLayout rl_all;
        private final LinearLayout ll_application_progress;
        ...
        private ViewHolder(@NonNull View itemView) {
            ...
            ll_application_progress = itemView.findViewById(R.id.ll_application_progress);
        }
    }
}
